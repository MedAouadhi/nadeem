 ...
 */
static bool smf_execute_all_entry_actions(struct smf_ctx *const ctx, const struct smf_state *new_state, const struct smf_state *topmost)
{
    ...
    for (const struct smf_state *to_execute = get_child_of(new_state, topmost); to_execute != NULL && to_execute != new_state;
...
void smf_set_state(struct smf_ctx *const ctx, const struct smf_state *new_state)
{
    ...
    if (new_state == NULL) {
        ESP_LOGE(TAG, "new_state cannot be NULL");
        return;
    }
     ...
     */
    if (internal->is_exit) {
        ESP_LOGE(TAG, "Cannot call %s from exit action", __func__);
        return;
    }
