...
|-----------|-----------------------|----------------|
| `bsp` | `bsp_init`, `bsp_sdcard_mount`, `bsp_sdcard_available`, `bsp_i2s_write`, `bsp_i2s_read_mic`, `bsp_set_volume`, `bsp_list_files` | Board pinout, codecs (ES7210 mic, ES8311 DAC), SD mount, I2S helpers. `bsp_sdcard_available()` returns true only when mount succeeded at boot. Specific to Waveshare ESP32-S3-AUDIO-Board. |
...
| `audio` | `audio_start`, `audio_current_state`, `audio_current_position`, `audio_set/get_volume`, `audio_set_stream_source`, `audio_clear_stream_source` | SMF task. Reacts to `NEV_AUDIO_PLAY/STOP/PAUSE/RESUME/NEXT/PREV`. Emits `NEV_AUDIO_STATE`, `NEV_AUDIO_TRACK_DONE`. Wraps `esp_audio_simple_player`. Supports `https://` and `raw://` URIs; `raw://` uses a registered read callback for Range-based streaming. |
| `semsem` | `semsem_start`, `semsem_trigger_uid` | SMF task. On placement: **SD mode** — cache check → manifest fetch → download → play from `/sdcard/semsem/<uid>/`. **Stream mode** (no SD) — manifest from NVS cache or backend (`nd_manifest` namespace), then stream each track via HTTPS with Range header for resume. Mode selected once at `semsem_start` from `bsp_sdcard_available()`. |
...
| `backend_client` | `backend_client_init`, `*_fetch_semsem_manifest`, `*_download_file`, `*_upload_stats`, `*_bootstrap`, `*_open_range`, `*_read_chunk`, `*_close_range` | HTTP client wrapped around `esp_http_client`. Bearer auth. `open_range` opens a GET with optional `Range: bytes=N-` header for stream-mode resumption; `read_chunk` matches `audio_stream_read_fn_t`. No TLS bundle wired (see [limitations.md](limitations.md)). |
...
main
 ...
 ├─ hmi ── nfc_pn532
 ├─ audio
 ├─ semsem ── backend_client, audio, bsp, stats (SD or stream, selected at boot)
